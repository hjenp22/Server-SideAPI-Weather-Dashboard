...
function searchCity(cityName) {
  fetch(
    `https://api.openweathermap.org/data/2.5/weather?q=${cityName}&units=imperial&appid=cfbfa377874ff036356a2f52cb66142c`
  )
      ...
      fetch(url)
        ...
        .then((data) => {
          ...
          for (let i = 6; i < data.list.length; i += 6) {
            displayForecastWeather(data.list[i], forecastDiv);
          }
        });
    });
}
...
function displayWeather(forecast, container) {
    ...
    forecastInfo.textContent = `Date: ${date.toLocaleDateString()},
        ...
        currentWeatherDiv.appendChild(forecastInfo);
  }

// Function to display weather information
function displayForecastWeather(forecast, container) {

  const forecastItem = document.createElement("div");
  forecastItem.setAttribute("class", "col-2");
  const { dt, main, wind, weather } = forecast;
  const date = new Date(dt * 1000);
  const { temp, humidity } = main;
  const { speed } = wind;
